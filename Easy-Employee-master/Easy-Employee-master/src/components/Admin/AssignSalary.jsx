...
const AssignSalary = () =>
{
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
          // Fetch employees and leaders data
          const emps = await getEmployees();
          const leaders = await getLeaders();
  
          // Safely handle and set employees data
          const employeeData = emps?.data?.length ? emps.data : []; // Fallback to empty array if no data
          const leaderData = leaders?.data?.length ? leaders.data : []; // Fallback to empty array if no data
  
          // Combine and set data
          setEmployees([...employeeData, ...leaderData]);
      } catch (error) {
          console.error("Error fetching employees or leaders:", error);
  
          // Handle errors gracefully by resetting employees
          setEmployees([]);
      }
  };
