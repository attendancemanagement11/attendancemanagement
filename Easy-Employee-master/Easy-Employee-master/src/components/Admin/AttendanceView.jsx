...
const AttendanceView = () => {
  ...
  useEffect(()=>{
    ...
    const fetchEmployees = async () => {
      try {
          const empObj = {}; // Initialize empObj here to avoid accidental scope issues
          ...
          if (emps?.data?.length) {
              emps.data.forEach(employee => {
                  empObj[employee?.id] = [employee?.name || "N/A", employee?.email || "N/A"];
              });
          }
  
          if (leaders?.data?.length) {
              leaders.data.forEach(leader => {
                  empObj[leader?.id] = [leader?.name || "N/A", leader?.email || "N/A"];
              });
          }
          ...
          // Optional: Show a user-friendly error message or set empty states
          setEmployeeMap({});
          setEmployees([]);
      }
  };
