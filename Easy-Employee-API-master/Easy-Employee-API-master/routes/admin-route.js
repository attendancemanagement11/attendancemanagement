...
// Geofence Routes (Admin handling geofences)
router.post('/geofence', asyncMiddleware(geofenceController.createGeofence)); // Create Geofence
router.patch('/geofence/:id', asyncMiddleware(geofenceController.updateGeofence)); // Update Geofence
router.get('/geofences', asyncMiddleware(geofenceController.findGeofences)); // List all Geofences
router.get('/geofence/:id', asyncMiddleware(geofenceController.findGeofenceById)); // Get Geofence by ID
router.delete('/geofence/:id', asyncMiddleware(geofenceController.deleteGeofence)); // Delete Geofence

// Task Routes (Admin handling tasks)
router.post('/task', asyncMiddleware(taskController.createTask)); // Create Task
router.post('/task/:id', asyncMiddleware(taskController.updateTask)); // Update Task
router.get('/task', asyncMiddleware(taskController.getTask)); // List all Tasks
router.get('/tasks', asyncMiddleware(taskController.getAllTasks)); // List all Tasks

router.get('/task/:id', asyncMiddleware(taskController.getTask)); // Get Task by ID
router.delete('/task/:id', asyncMiddleware(taskController.deleteTask)); // Delete Task
